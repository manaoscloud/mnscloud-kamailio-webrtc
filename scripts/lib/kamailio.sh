...
render_kamailio_sip_targets() {
...

  {
    ...
    printf '    xlog("L_INFO", "MNSCloud WebRTC route request method=$rm ruri=$ru rd=$rd from=$fu fd=$fd to=$tu td=$td source=$si\\\\n");\n\n'
    ...
    if [[ -s "$config_file" ]]; then
      jq -r '(.sipTargets // .data.sipTargets // []) | .[] | @base64' "$config_file" | while IFS= read -r encoded; do
        ...
        route_condition="\$rd == \"${domain}\" || \$fd == \"${domain}\" || \$td == \"${domain}\""
        if [[ -n "$sip_domain" && "$sip_domain" != "$domain" ]]; then
          route_condition="${route_condition} || \$rd == \"${sip_domain}\" || \$fd == \"${sip_domain}\" || \$td == \"${sip_domain}\""
        fi

        printf '    if (%s) {\n' "$route_condition"
        printf '        $du = "%s";\n' "$target_uri"
        printf '        xlog("L_INFO", "MNSCloud WebRTC routing rd=$rd fd=$fd td=$td web_domain=%s sip_domain=%s type=%s target=%s\\\\n");\n' "$domain" "${sip_domain:-$domain}" "$target_type" "$target_uri"
...
render_kamailio_path_relay() {
  local output_file="$KAMAILIO_MNS_DIR/mnscloud-path-relay.cfg"
  local webrtc_media_flags="replace-origin replace-session-connection RTP/SAVPF ICE=force DTLS=passive SDES-off rtcp-mux-offer rtcp-mux-accept no-rtcp-attribute codec-mask-G729 via-branch=extra"
  cat > "$output_file" <<'EOF'
# This file is generated by MNSCloud. Do not edit manually.

route[MNSCLOUD_PATH_RELAY] {
    if (loose_route()) {
        xlog("L_INFO", "MNSCloud WebRTC Path relay method=$rm ruri=$ru route=$hdr(Route) source=$si\n");
        if (is_method("INVITE") && has_body("application/sdp")) {
            $avp(mns_rtp_branch) = "mns-webrtc";
            xlog("L_INFO", "MNSCloud WebRTC Path media offer callid=$ci source=$si ruri=$ru\n");
EOF
  cat >> "$output_file" <<EOF
            if (!rtpengine_offer("$webrtc_media_flags")) {
EOF
  cat >> "$output_file" <<'EOF'
