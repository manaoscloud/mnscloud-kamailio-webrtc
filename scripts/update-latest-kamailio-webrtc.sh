...
usage() {
  cat <<'EOF'
Usage:
  sudo ./scripts/update-latest-kamailio-webrtc.sh [--api-base https://dev.publichost.cloud/api/v1] [--channel stable] [--print-command]

Resolves the latest approved mnscloud-kamailio-webrtc release automatically, then applies it.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --help|-h) usage; exit 0 ;;
    *) printf '[mnscloud-kamailio-webrtc] ERROR: unknown argument: %s\n' "$1" >&2; usage >&2; exit 1 ;;
  esac
done
...
resolve_ref() {
  ...
  local ref
  ref="$(
    MNSCLOUD_RELEASE_URL="${API_BASE}/runtime/releases/latest?product=mnscloud-kamailio-webrtc&channel=${CHANNEL}" python3 <<'PY' 2>/dev/null || true
import json
import os
...
[[ -n "$RELEASE_REF" ]] || { printf '[mnscloud-kamailio-webrtc] ERROR: no release tag found\n' >&2; exit 1; }
printf '[mnscloud-kamailio-webrtc] latest release: %s\n' "$RELEASE_REF"
...
"$REPO_ROOT/scripts/update-kamailio-webrtc.sh"
